...
if (!source || !destination) {
  console.error(
    "Usage: node deploy-config.js <source_network> <destination_network>"
  );
  process.exit(1);
}
...
function deployAndCapture(network, isSource) {
  ...
  if (!allowedNetworks.includes(network)) {
    console.error(
      "Invalid network. Please provide a valid network as an argument."
    );
    return;
  }
  exec(
    ...
    (error, stdout, stderr) => {
      ...
      const match = output.match(
        /Contract (\S+) deployed to (\S+) on network (\S+)/
      );
      ...
      `);
        ...
        console.log(
          ...
          } with address ${address} on network ${network}`
        );
      } else {
        console.error(
          "❌ Could not find contract address and network in output"
        );
      }
    }
  );
